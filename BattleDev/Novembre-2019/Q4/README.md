...
Il s'agit littéralement du problème du sac à dos (_knapsack problem_). Je vous invite à lire l'énoncé pour plus de détails. La solution que je vais vous proposer utilise un **algorithme glouton**.
_Notez que cette solution n'est pas parfaite (elle fait comme s'il était possible de découper les pierres). Toutefois, elle est suffisante pour réussir à valider l'exercice._
...
Je vous propose de créer une structure qui permettra de représenter les produits, que ce soient des pierres ou des poudres. En effet, il n'est pas nécessaire de faire la distinction entre les deux pour résoudre le problème (c'est d'ailleurs un défaut de cette solution).
```cpp
struct  Product {
...
* tant que le sac n'est pas plein et qu'il reste des produits
    * prendre le produit qui a la plus grande valeur par gramme et en mettre un maximum dans la lampe

Avant de vous lancer dans l'écriture de l'algorithme, je vous propose de trier les produits par ordre croissant de valeur par gramme. En effet, puisqu'à chaque tour de boucle il faudra chercher le produit qui a la plus grande valeur par gramme, ce sera beaucoup plus facile sur un vecteur préalablement trié.
...
* Pour en mettre un maximum dans la lampe
    1) soit la place manque, auquel cas on remplira la lampe avec le produit (_pour rappel, cette solution fait comme s'il était possible de couper les pierres_)
    2) soit il reste suffisamment de place dans la lampe pour le mettre en entier

Ainsi, le code ressemble à ceci:
```cpp
while (C > 0 && !p.empty()) {   // tant qu'il reste de la place dans la lampe, on a C > 0
    if (p.back().q >= C) {      // Cas 1: il y a plus de produit que de place
        ans = ans + (C * p.back().v);
        C = 0;
    }
    else {                      // Cas 2: le produit tient en entier dans la lampe
...
L'algorithme est complet, vous affichez bien une réponse sur la sortie standard mais... ce n'est pas la bonne ! Et pour cause: jusqu'à présent, tout le code utilise des entiers (```int```), d'où une approximation lorsqu'on calcule la valeur par gramme des pierres. Comme j'étais en temps limité, je ne me suis pas embêtée à caster, et j'ai tout passé en ```double```, ce qui donne:
```cpp
#include <algorithm>
#include <iostream>
#include <vector>
...
```
**Je vous rappelle que cette solution n'est pas parfaite, et peu parfois vous être refusée.** En effet, ici, les pierres sont considérées comme des poudres et peuvent ainsi être coupées, or ce n'est pas le cas.